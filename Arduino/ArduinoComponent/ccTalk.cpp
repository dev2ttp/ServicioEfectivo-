...
void ccTalk::device_reset(){
  cctsend(reset_device, ADDRESS, 0); //ben added because credit buffer stays if power isn't reset
  while (RX_state < RXcomplete) {
    ccTalkReceive(); // wait for an answer or error
  }
...
void ccTalk::device_init(){
  ...
  if (RX_state != RXcomplete) {
    //Serial.print("Error: ");
    //Serial.println(RX_msg[RX_state]);
  }
  ...
  for (int i = 0; i < 16 ; i++) {
    coin_value[i] = 0; // Clean the coin value array
    coin_credits[i] = 0;
  }
  ...
  for (unsigned char i = 1; i < 17; i++) {
    ...
    cctsend(request_coin_id, ADDRESS, 1, i);
    while (RX_state < RXcomplete) {
      ccTalkReceive();
    }
    if (RX_state != RXcomplete) {
  ...
  cctsend(modify_inhibit_status, ADDRESS, 2, 255, 255); //just enable all channels
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
  ...
  // Habilita todos los canales, el motor se enciende
  /*cctsend(modify_master_inhibit_status, ADDRESS, 1, 1);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }*/
  // if (RX_state != RXcomplete) //error handling
...
char * ccTalk::get_master_inhibit(){
  ...
  cctsend(request_master_inhibit_status, ADDRESS, 0);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
...
char * ccTalk::getAddress() {
  //Serial.println("Getting address poll");
  cctsend(address_poll, 0, 0);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling

  // TODO: create another function to parse this message
  return printBINdata();
}

char * ccTalk::getSerialNumber() {
  cctsend(request_serial_number, ADDRESS, 0);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
  if (RX_state == RXcomplete) {
    int i, j, len = RX_buffer[1] + 4;
    unsigned long sn = 0, p;
    for (i = 4, p = 1; i < len; i++) {
      sn += (RX_buffer[i] * p);
      p *= 256;
    }

    sprintf(serialNumber, "%ld", sn);
  }
  
  return serialNumber;
}

char * ccTalk::getEquipmentCategory() {
  cctsend(request_equipment_category_id, ADDRESS, 0);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
...
unsigned int* ccTalk::getCoinValues() {
  return coin_value;
}

unsigned int* ccTalk::getCoinCredits() {
  return coin_credits;
}

unsigned int ccTalk::getCoinEventCounter() {
  return coineventcounter;
}

char * ccTalk::getHopperCoinValue() {
  cctsend(request_hopper_coin_value, ADDRESS, 1);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
...
void ccTalk::master_inhibit_on(){
  //Serial.println("Setting master inhibit");
  cctsend(modify_master_inhibit_status, ADDRESS, 1, 0);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
...
char * ccTalk::get_inhibit(){
  ...
  cctsend(request_inhibit_status, ADDRESS, 0);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
...
void ccTalk::inhibit_on(){
  cctsend(modify_inhibit_status, ADDRESS, 2, 0, 0); //just disable all channels
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
...
void ccTalk::diagnostic(){
  ...
  cctsend(request_manufacturer_id, ADDRESS, 0);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
  ...
  cctsend(request_product_code, ADDRESS, 0);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
  ...
  cctsend(request_software_revision, ADDRESS, 0);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
...
int ccTalk::read_credit(){
  cctsend(read_buffered_credit_or_error_codes, ADDRESS, 0);
    while (RX_state < RXcomplete) {
      ccTalkReceive();
    }
    if (RX_state != RXcomplete) {
    ...
    credit = 0;
    //for (uint8_t i = 0; i < MAX_COIN_CHANNELS; i++) coin_credits[i] = 0;
    ...
    if (buffered_events > 0) {
      ...
      strcpy(szBuf, "<<<");
      for (int i = 0; i < 11; i++){
          sprintf(&szBuf[i*3+3], " %02X", RX_buffer[i+4]);
      }
    ...
    while (buffered_events > 0) {// Read buffered events one by one
      ...
      if (RX_buffer[(buffered_events << 1) + 5] == 0) { // event A = 0 means an error or coin rejected
        return -1;
        ...
        coinIndex = RX_buffer[(buffered_events << 1) + 5] - 1;
        credit += coin_value[coinIndex];
        coin_credits[coinIndex]++;
